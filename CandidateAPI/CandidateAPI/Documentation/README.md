...
Method: POST
Description: Adds a new candidate or updates an existing candidate.
Request Body:
JSON
...
Retrieving a candidate by email
Retrieving all candidates
Logging
Logging is configured using Serilog. Logs are written to a file located at logs/log.txt with daily rolling intervals.
...
1. Right-click on project in Solution Explorer and select **"Manage NuGet Packages"**.
2. Search for and install the following packages:
   - `Microsoft.EntityFrameworkCore.SqlServer`
   - `Microsoft.EntityFrameworkCore.Tools`

### 2. Update the Database Context Configuration
1. Open `Program.cs` and update the database context configuration to use SQL Server instead of SQLite.
